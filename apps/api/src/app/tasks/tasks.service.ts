...
export class TasksService {
  ...
  async createTasks(runId: string, createTasksDto: CreateTasksDto): Promise<Task[]> {
    ...
    const previousRun = await this.getPreviousRun(run.pullRequest);
  ...
  private async getPreviousRun(pullRequest: PullRequest): Promise<Run> {
    return this.runModel.findOne({ pullRequest: pullRequest._id }).sort({ endedAt: -1 });
  }
