...
export class PendingTasksService {
  ...
  async startPendingTask(runId: number, startTaskDto: StartTaskDto): Promise<StartTaskResponseDto> {
    ...
    return await this.dataSource.transaction(async (manager) => {
      ...
      for (const taskPriority of run.prioritization) {
        ...
        const RunningPendingTasks = tasks.filter((task) => task.status === TaskStatus.Running || task.status === TaskStatus.Pending);
  ...
  private checkDependencies(
    ...
    if (!task) {
      console.error("TAsk not defined. At this point task must be defined");
      return null;
    }
